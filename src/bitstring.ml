...
let make_bitstring len c =
  if len >= 0
  then Bytes.make ((len + 7) lsr 3) c, 0, len
  else invalid_arg (sprintf "make_bitstring/create_bitstring: len %d < 0" len)
;;
...
module I = struct
  ...
  let mask bits =
    if bits < 30 || (bits < 32 && Sys.word_size = 64)
    then (one <<< bits) - 1
    else if bits = 30
    then max_int
    else if bits = 31
    then minus_one
    else invalid_arg "Bitstring.I.mask"
  ;;
  ...
  let range_signed v bits =
    if v >= zero
    then range_unsigned v bits
    else if bits = 31 && Sys.word_size = 32
    then v >= min_int
...
module I32 = struct
  ...
  let mask bits =
    ...
    then minus_one
    else invalid_arg "Bitstring.I32.mask"
  ;;
...
module I64 = struct
  ...
  let mask bits =
    ...
    then minus_one
    else invalid_arg "Bitstring.I64.mask"
  ;;
...
let _get_byte32 data byteoff strlen =
  if strlen > byteoff then Int32.of_int (Char.code (Bytes.get data byteoff)) else 0l
;;
  ...
  let word =
    ...
    then (
      let word =
        ...
        + (_get_byte data (byteoff + 3) strlen lsr 1)
      in
      word lsr (31 - flen))
    else if flen <= 24
    then (
      ...
      let word =
        ...
        (c0 lsl 23) + (c1 lsl 15) + (c2 lsl 7) + c3
      in
      word lsr (31 - flen))
  in
  word (*, off+flen, len-flen*)
...
let extract_int_le_signed = extract_and_extend_sign extract_int_le_unsigned

let extract_int_ne_unsigned =
  if nativeendian = BigEndian then extract_int_be_unsigned else extract_int_le_unsigned
;;

let extract_int_ne_signed = extract_and_extend_sign extract_int_ne_unsigned
...
let extract_int_ee_signed e = extract_and_extend_sign (extract_int_ee_unsigned e)

let _make_int32_be c0 c1 c2 c3 =
  Int32.logor
    (Int32.logor
       (Int32.logor (Int32.shift_left c0 24) (Int32.shift_left c1 16))
...
let _make_int32_le c0 c1 c2 c3 =
  Int32.logor
    (Int32.logor
       (Int32.logor (Int32.shift_left c3 24) (Int32.shift_left c2 16))
    ...
    (Int64.logor
       (Int64.logor
          (Int64.logor
             (Int64.logor
                (Int64.logor
                   (Int64.logor (Int64.shift_left c0 56) (Int64.shift_left c1 48))
  ...
  let word =
    ...
    then (
      let word =
        ...
        _make_int64_be c0 c1 c2 c3 c4 c5 c6 c7
      in
...
module Buffer = struct
  ...
  let contents { buf; len; last } =
    let data =
      if len land 7 = 0
      then Buffer.to_bytes buf
      else Bytes.cat (Buffer.to_bytes buf) (Bytes.make 1 (Char.chr last))
    in
    data, 0, len
  ;;
    ...
    then
      ...
      then (
        ...
        then
          ...
          let last = Char.code (Bytes.get str lastidx) in
          (* last char *)
...
let construct_bit buf b _ _ = Buffer.add_bit buf b
...
let construct_char_unsigned buf v flen exn =
  ...
  if flen = 8 then Buffer.add_byte buf v else Buffer._add_bits buf v flen
;;

let construct_char_signed buf v flen exn =
  let max_val = 1 lsl flen
  and min_val = -(1 lsl pred flen) in
  if v < min_val || v >= max_val then raise exn;
  if flen = 8
  then Buffer.add_byte buf (if v >= 0 then v else 256 + v)
  else Buffer._add_bits buf v flen
;;
...
let construct_int_be_unsigned = construct_int I.range_unsigned I.map_bytes_be
let construct_int_be_signed = construct_int I.range_signed I.map_bytes_be
let construct_int_le_unsigned = construct_int I.range_unsigned I.map_bytes_le
let construct_int_le_signed = construct_int I.range_signed I.map_bytes_le
...
let construct_int_ne_signed =
  if nativeendian = BigEndian then construct_int_be_signed else construct_int_le_signed
;;
...
let construct_int32_be_unsigned buf v flen _ =
  Buffer.add_byte buf (Int32.to_int (Int32.shift_right_logical v 24));
  Buffer.add_byte
    buf
    (Int32.to_int (Int32.logand (Int32.shift_right_logical v 16) 0xff_l));
  Buffer.add_byte buf (Int32.to_int (Int32.logand (Int32.shift_right_logical v 8) 0xff_l));
  Buffer.add_byte buf (Int32.to_int (Int32.logand v 0xff_l))
;;
...
let construct_bitstring buf (data, off, len) =
  ...
  let data =
    ...
    if off = 0 then data else Bytes.sub data off (Bytes.length data - off)
  in
...
let equals ((_, _, len1) as bs1) ((_, _, len2) as bs2) =
  if len1 <> len2 then false else if bs1 = bs2 then true else 0 = compare bs1 bs2
;;
...
let is_zeroes_bitstring ((data, off, len) as bits) =
  ...
  then (
    let off = off lsr 3
    and len = len lsr 3 in
    let rec loop i =
      if i < len
      then if Bytes.unsafe_get data (off + i) <> '\000' then false else loop (i + 1)
      else true
    in
    loop 0)
  else (
...
let is_ones_bitstring ((data, off, len) as bits) =
  ...
  then (
    ...
    let rec loop i =
      if i < len
      then if Bytes.unsafe_get data (off + i) <> '\xff' then false else loop (i + 1)
      else true
    in
    loop 0)
  else (
...
let is_prefix ((b1, o1, l1) as bs1) ((b2, o2, l2) as bs2) =
  ...
  if l2 > l1 || l1 = 0 || l2 = 0
  then false (* Use the fast path if the bitstrings are aligned *)
  else if o1 land 7 = o2 land 7
  then is_prefix_fastpath b1 o1 b2 o2 l2
...
let put (data, off, len) n v =
  if n < 0 || n >= len
  then index_out_of_bounds ()
  else (
    let i = off + n in
    let si = i lsr 3
    and mask = 0x80 lsr (i land 7) in
