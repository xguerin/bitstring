...
let match_random_bits_with_int_test _ =
  ...
  for len = 1 to 99 do
    for bitlen = 1 to 63 do
      ...
      List.iter
        (fun i ->
           Bitstring.construct_int64_be_unsigned
             bits
             i
             bitlen
      ...
      let actual =
        let rec loop bits =
          ...
          | {| _ |} ->
            failwith (sprintf "loop failed with len = %d, bitlen = %d" len bitlen)
        in
        loop bits
      in
      assert_equal actual expected
...
let signed_byte_create_and_match_test _ =
  ...
  in
  let t s i = List.fold_left (fun ok n -> s n = n && ok) true (a i) in
  let ok =
    fst
      (List.fold_left
         (fun (ok, i) s -> t s i && ok, succ i)
         (true, 2)
         [ (fun n ->
             match%bitstring [%bitstring {| n : 2 : signed |}] with
             | {| i : 2 : signed |} -> i
             | {| _ |} -> assert false)
         ; (fun n ->
             match%bitstring [%bitstring {| n : 3 : signed |}] with
             | {| i : 3 : signed |} -> i
             | {| _ |} -> assert false)
         ; (fun n ->
             match%bitstring [%bitstring {| n : 4 : signed |}] with
             | {| i : 4 : signed |} -> i
             | {| _ |} -> assert false)
         ; (fun n ->
             match%bitstring [%bitstring {| n : 5 : signed |}] with
             | {| i : 5 : signed |} -> i
             | {| _ |} -> assert false)
         ; (fun n ->
             match%bitstring [%bitstring {| n : 6 : signed |}] with
             | {| i : 6 : signed |} -> i
             | {| _ |} -> assert false)
         ; (fun n ->
             match%bitstring [%bitstring {| n : 7 : signed |}] with
             | {| i : 7 : signed |} -> i
             | {| _ |} -> assert false)
         ; (fun n ->
             match%bitstring [%bitstring {| n : 8 : signed |}] with
             | {| i : 8 : signed |} -> i
             | {| _ |} -> assert false)
         ])
  in
    ...
    List.fold_left
      (fun (ok, i) (b, m) ->
         ...
         let gut5 =
           ...
           let test () =
             let signed_number =
               Int32.to_int (Int32.add (Random.int32 plage) (Int32.of_int minp))
             in
...
let fixed_extraction_test _ =
  for i = 0 to 129 do
    ...
    | {| _ : i : bitstring;
      ...
      then (
        eprintf
          "15_extract_int: match failed %b %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d \
           %d %d %d %ld %ld %ld %Ld %Ld %Ld %Ld %Ld %Ld\n"
          a
          b0
          b1
          b2
...
let bitstring_of_int32 i = [%bitstring {| i : 32 |}]
let bitstring_of_int64 i = [%bitstring {| i : 64 |}]
...
let rand64 () =
  let r0 = Int64.shift_left (Int64.of_int (Random.bits ())) 34 in
  (* 30 bits *)
  let r1 = Int64.shift_left (Int64.of_int (Random.bits ())) 4 in
  (* 30 bits *)
  let r2 = Int64.of_int (Random.int 16) in
  (* 4 bits *)
...
let mask64 bits =
  if bits < 63
  then Int64.pred (Int64.shift_left 1L bits)
  else if bits = 63
  then Int64.max_int
  else if bits = 64
  then -1L
  ...
  for pass = 0 to nr_passes - 1 do
    ...
    let bits =
      ...
      | Bitstring.Construct_failure (msg, _, _, _) ->
        ...
        dump n0 n0sz n1 n1sz n2 n2sz n3 n3sz Bitstring.empty_bitstring 0L 0L 0L 0L;
        exit 2
    in
    let r0, r1, r2, r3 =
      match%bitstring bits with
...
let nasty_non_aligned_corner_case_test _ =
  ...
  for offset = 0 to 65 do
    for len = 1 to 65 do
      ...
      if Bytes.of_string actual <> expected
      then (
...
let subbitstring_test _ =
  ...
  for off = 0 to 65 do
    for len = 65 - off to 0 do
      ...
      for i = 0 to len - 1 do
        if Bitstring.get bits (off + i) <> Bitstring.get sub i
        then (
...
let file_load_test _ =
  ...
  let filename, chan =
    Filename.open_temp_file ~mode:[ Open_binary ] "bitstring_test" ".tmp"
  in
...
let zeroes_ones_test _ =
  for i = 0 to 33 do
    ...
    if not (Bitstring.is_zeroes_bitstring bits)
    then (
      eprintf "is_zeros_bitstring failed %d\n" i;
      exit 1);
    if i > 0 && Bitstring.is_ones_bitstring bits
    then (
  ...
  for i = 0 to 33 do
    ...
    if not (Bitstring.is_ones_bitstring bits)
    then (
      eprintf "is_ones_bitstring failed %d\n" i;
      exit 1);
    if i > 0 && Bitstring.is_zeroes_bitstring bits
    then (
    ...
    | (e, expected) :: rest ->
      ...
      (match%bitstring bits with
       | {| actual : 32 : endian (e);
         ...
         ->
         if actual <> expected
         then failwith (sprintf "actual %ld <> expected %ld" actual expected)
       | {| _ |} as bits ->
         Bitstring.hexdump_bitstring stderr bits;
         exit 1);
      loop rest
...
let simple_offset_test _ =
  ...
  let test_bits bits i n j m k =
    ...
    | {| i' : 8;
      ...
      when i = i' && j = j' && k = k' -> () (* ok *)
    | {| _ |} ->
      failwith (sprintf "60_simple_offset: test_bits: failed %d %d %d %d %d" i n j m k)
  in
  for n = 8 to 128 do
    for m = n + 8 to 256 do
      List.iter
        (fun (i, j, k) -> test_bits (make_bits i n j m k) i n j m k)
...
let offset_string_test _ =
  ...
  let test_bits bits si n sj m sk =
    ...
    | {| _ |} ->
      failwith (sprintf "61_offset_string: test_bits: failed %S %d %S %d %S" si n sj m sk)
  in
  for n = 64 to 128 do
    for m = n + 8 to 256 do
      List.iter
        (fun (si, sj, sk) -> test_bits (make_bits si n sj m sk) si n sj m sk)
        [ "ABCDEFGH", "x", "HGFEDCBA"
        ; "01234567", "0", "76543210"
...
let computed_offset_test _ =
  ...
  let test_bits bits p i n j m k =
    ...
    | {| i' : 8
      ...
      Printf.printf "%x %x\n" k k';
      assert_equal k k'
    | {| _ |} -> assert_failure "Bitstring parsing failure"
  in
  for p = 1 to 4 do
    for n = 8 to 128 do
      for m = n + 8 to 256 do
        List.iter
          (fun (i, j, k) -> test_bits (make_bits p i n j m k) p i n j m k)
          [ 0x55, 0xaa, 0x33; 0x33, 0xaa, 0x55; 0x12, 0x34, 0x56 ]
      done
    done
  done
...
let save_offset_to_test _ =
  ...
  let test_bits bits p i n j m k =
    ...
    | {| i' : 8;
      ...
      when i = i' && j = j' && k = k' && j_offset = n && k_offset = m -> () (* ok *)
    | {| _ |} ->
      failwith
        (sprintf "65_save_offset_to: test_bits: failed %d %d %d %d %d %d" p i n j m k)
  in
  for p = 0 to 4 do
    for n = 8 to 64 do
      for m = n + 8 to 128 do
...
let check_bind_test _ =
  ...
  | {| i : 16 : check (i = 101), bind (i*4);
       j : 16 : check (j = 202) |}
    ->
    if i <> 404 || j <> 202 then failwith (sprintf "70_check_and_bind: failed: %d %d" i j)
    ...
    (fun (filename, n, bits) ->
       ...
       let cmd =
         sprintf
           ...
           (Filename.quote actual_filename)
       in
...
let concat_regression_test _ =
  ...
  let bs5 =
    Bitstring.concat
      [ Bitstring.subbitstring bs_256 0 66; Bitstring.subbitstring bs_256 66 67 ]
  in
  ...
  let bs6 =
    Bitstring.concat
      [ Bitstring.subbitstring bs_256 0 64; Bitstring.subbitstring bs_256 64 64 ]
  in
  ...
  let bs7 =
    Bitstring.concat
      [ Bitstring.subbitstring bs_256 0 65; Bitstring.subbitstring bs_256 65 64 ]
  in
